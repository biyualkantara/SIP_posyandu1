...
function submit() {
  ...
  try {
    form.rows.forEach((row, index) => {
      ...
      if (!row.id_imun) {
        openError(`Data ke-${index + 1}: Jenis imunisasi wajib dipilih`)
        throw new Error('invalid')
      }
      if (!row.tgl_imun) {
        openError(`Data ke-${index + 1}: Tanggal imunisasi wajib diisi`)
        throw new Error('invalid')
      }
    })
  } catch {
...
<template>
  <div class="page-wrapper">
    ...
    <div class="page-header">
      <div>
        <h2 class="mb-1">Tambah Imunisasi Ibu Hamil (BUMIL)</h2>
        <p class="text-muted">Input multi data imunisasi ibu hamil</p>
      </div>
      <Link href="/posyandu/bumil-imun" class="btn btn-outline-secondary">
