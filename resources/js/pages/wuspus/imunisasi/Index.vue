  ...
  router.delete(`/posyandu/wuspus-imun/${selected.value.id_wuspus_imun}`, {
    ...
    onSuccess: () => {
      ...
      window.dispatchEvent(new CustomEvent("toast", {
        detail: { type: "success", message: "Data imunisasi WUS/PUS berhasil dihapus!" }
      }))
    },
    onError: () => {
      window.dispatchEvent(new CustomEvent("toast", {
  ...
  <AdminLayout>
    <div class="mt-3 p-4 bg-white main-container">
      <div class="header-flex mb-3">
        <h1>Imunisasi WUS/PUS</h1>
        <Link href="/posyandu/wuspus-imun/create" class="btn btn-primary">+ Tambah</Link>
      </div>
      ...
      <DataTable :columns="columns" :rows="rows" :perPage="10">
        ...
        <template #col-actions="{ row }">
          <Link :href="`/posyandu/wuspus-imun/${row.id_wuspus_imun}`">
            <span class="bg-info p-3 mr-2 rounded-circle text-white" style="cursor:pointer;">
              <i class="icon-eye"></i>
            </span>
          </Link>

          <Link :href="`/posyandu/wuspus-imun/${row.id_wuspus_imun}/edit`">
    ...
    <div v-if="modalOpen" class="overlay-blur" @click.self="closeModal">
      <div class="modal-card">
        <h3 class="w-100 text-center">Hapus Data Imunisasi?</h3>
        <hr>
        <div class="text-center">
          ...
          <div class="mt-2">{{ selected.jns_imun }}</div>
          <div class="mt-1">Tanggal: <b>{{ selected.tgl_imun }}</b></div>
        </div>
