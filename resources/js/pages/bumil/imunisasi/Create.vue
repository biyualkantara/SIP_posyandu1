...
function submit() {
  ...
  try {
    form.rows = form.rows.map((row, index) => {
      ...
      if (!row.id_imun)   { openError(`Data ke-${index + 1}: Jenis imunisasi wajib dipilih`); throw new Error('invalid') }
      if (!row.tgl_imun)  { openError(`Data ke-${index + 1}: Tanggal imunisasi wajib diisi`); throw new Error('invalid') }
      return { ...row }
    })
  } catch { return }
  ...
  form.post('/posyandu/bumil-imun/store-multiple', {
    preserveScroll: true,
    onSuccess: () => {
      openSuccess('Data imunisasi bumil berhasil disimpan')
      setTimeout(() => router.visit('/posyandu/bumil-imun'), 700)
    },
  ...
  <AdminLayout>
    <div class="bg-white p-4 main-container">
      <div class="header-flex mb-3">
        <h2>Tambah Imunisasi Ibu Hamil (BUMIL)</h2>
        <Link href="/posyandu/bumil-imun" class="btn btn-secondary">← Kembali</Link>
      </div>
      ...
      <form @submit.prevent="submit">
        ...
        <Table class="mt-4">
          <TableHead>
            <TableRow>
              <TableCol asHead>Form Imunisasi BUMIL</TableCol>
