...
function validateRow(row, index) {
  ...
  if (!row.id_wuspus) errors.push('Ibu harus dipilih')
  if (!row.nama_bayi) errors.push('Nama bayi harus diisi')
  if (!row.tgl_lhr) errors.push('Tanggal lahir harus diisi')
  if (!row.jk) errors.push('Jenis kelamin harus dipilih')
...
function submit() {
  ...
  if (ibuIds.length !== uniqueIbuIds.length) {
    ...
    allErrors.push({
      row: 'Semua',
      errors: ['Tidak boleh memilih ibu yang sama untuk data bayi yang berbeda']
    })
  }
  ...
  form.post('/posyandu/bayi/store-multiple', {
    ...
    onSuccess: () => {
      hideToast()
      openSuccess(`${form.rows.length} data bayi berhasil disimpan`)
      setTimeout(() => {
        router.visit('/posyandu/bayi')
      }, 1500)
    },
    onError: (errors) => {
      hideToast()
  ...
  <div class="data-container">
    ...
    <div class="header-section">
      <div class="header-left">
        <h1 class="page-title">Tambah Biodata Bayi</h1>
        <p class="page-subtitle">Input multi data bayi</p>
      </div>
    ...
    <div class="main-card">
      <div class="card-body">
        ...
        <div v-if="form.posyandu_id" class="info-alert">
          <span>ℹ️</span>
          <span>
            Total ibu tersedia: <strong>{{ ibuOptions.length }}</strong> | 
            Terpilih: <strong>{{ selectedIbuIds.length }}</strong> | 
            Sisa: <strong>{{ ibuOptions.length - selectedIbuIds.length }}</strong>
          </span>
        </div>
        ...
        <form @submit.prevent="submit">
          ...
          <div v-if="form.posyandu_id" class="data-rows-section">
            <div class="section-header">
              <h6 class="section-title">Data Bayi</h6>
              <div class="section-badge">
                <span class="badge-total">Total: {{ form.rows.length }} data</span>
                <span class="badge-available">Ibu tersedia: {{ ibuOptions.length - selectedIbuIds.length }}</span>
              </div>
            </div>
            ...
            <div v-for="(row, i) in form.rows" :key="i" class="data-card">
              ...
              <div class="form-grid-3">
                ...
                <div class="form-field">
                  ...
                  <select class="form-input" v-model="row.jk">
                ...
                <div class="form-field">
                  ...
                  <small v-if="getIbuOptionsForRow(i).length === 0" class="text-warning">
                    ⚠️ Tidak ada ibu tersedia untuk baris ini
                  </small>
                </div>
              </div>
