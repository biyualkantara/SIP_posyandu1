...
function confirmDelete() {
    if (!selected.value?.id_bumil) {
        showToast('error', 'Data tidak valid')
        closeModal()
        return
    }

    const namaBumil = selected.value.nama_wuspus || 'Data ibu hamil'
    saveScrollPosition()
    ...
    router.delete(`/posyandu/bumil/${selected.value.id_bumil}`, {
        ...
        onSuccess: (page) => {
            ...
            if (page.props.flash?.success) {
                ...
                // Fallback message
                showToast('success', `Data "${namaBumil}" berhasil dihapus!`)
            }
  ...
  <AdminLayout>
    ...
    <div class="data-container">
        ...
        <div class="header-section">
            <div class="header-left">
                <h1 class="page-title">Data Ibu Hamil (BUMIL)</h1>
                <p class="page-subtitle">Kelola data ibu hamil di wilayah Anda</p>
            </div>
        ...
        <div class="table-section">
            ...
            <div v-if="rows.length === 0" class="empty-state">
                ...
                <p class="empty-description">Data ibu hamil belum tersedia. Silakan tambah data baru.</p>
                <Link href="/posyandu/bumil/create" class="btn-create empty-btn" @click="handleLinkClick">
                    <i class="icon-plus"></i>
            ...
            <DataTable v-else :columns="columns" :rows="rows" :perPage="10">
                ...
                <template #col-id_bumil="{ row }">
                    <span class="id-badge">#{{ row.id_bumil }}</span>
                </template>
                ...
                <template #col-tgl_daftar="{ row }">
                    <span class="tanggal-badge">{{ formatDate(row.tgl_daftar) }}</span>
                </template>

                <template #col-umur_kehamilan="{ row }">
                    <span v-if="row.umur_kehamilan" class="info-badge">
                        {{ row.umur_kehamilan }} Minggu
                    </span>
                    <span v-else class="text-muted">-</span>
                </template>

                <template #col-hamil_ke="{ row }">
                    <span v-if="row.hamil_ke" class="info-badge">
                        Ke-{{ row.hamil_ke }}
                    </span>
                    <span v-else class="text-muted">-</span>
                </template>

                <template #col-lila="{ row }">
                    <span v-if="row.lila" class="info-badge">
                        {{ row.lila }} cm
                    </span>
                    <span v-else class="text-muted">-</span>
                </template>
                ...
                <template #col-actions="{ row }">
                    <div class="action-group">
                        <Link 
                            :href="`/posyandu/bumil/${row.id_bumil}`"
                            @click="handleLinkClick"
                        >
                        ...
                        <Link 
                            :href="`/posyandu/bumil/${row.id_bumil}/edit`"
                            @click="handleLinkClick"
                        >
    ...
    <div v-if="modalOpen" class="modal-overlay" @click.self="closeModal">
        <div class="modal-content">
            ...
            <h3 class="modal-title">Hapus Data Ibu Hamil?</h3>
