...
const columns = [
  ...
  { key: 'tgl_pnb', label: 'Tanggal', sortable: true },
...
function confirmDelete() {
  router.delete(`/posyandu/bayi-pnb/${selected.value.id_bayi_pnb}`, {
    preserveScroll: true,
    onSuccess: () => {
      closeModal()
      router.reload({ only: ['data'] })
      window.dispatchEvent(new CustomEvent("toast", {
        detail: { type: "success", message: "Data penimbangan bayi berhasil dihapus!" }
      }))
    },
    onError: () => {
      window.dispatchEvent(new CustomEvent("toast", {
...
<template>
  ...
  <div class="data-container">
    ...
    <div class="header-section">
      <div>
        <h1 class="page-title">Penimbangan Bayi</h1>
        <p class="page-subtitle">Kelola data penimbangan bayi posyandu</p>
      </div>
      ...
      <DataTable v-else :columns="columns" :rows="rows" :perPage="20">
        ...
        <template #col-actions="{ row }">
          <div class="action-group">
            ...
            <Link :href="`/posyandu/bayi-pnb/${row.id_bayi_pnb}`">
              <span class="action-btn btn-show">
                <i class="icon-eye"></i>
              </span>
            </Link>

            <Link :href="`/posyandu/bayi-pnb/${row.id_bayi_pnb}/edit`">
