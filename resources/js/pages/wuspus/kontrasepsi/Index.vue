...
console.log('Data Kontrasepsi WUS/PUS:', props.data)
...
const saveScrollPosition = () => {
  ...
  sessionStorage.setItem('scrollPosition_wuspus_kontrasepsi', scrollPosition.value)
}

// Restore posisi scroll setelah kembali
const restoreScrollPosition = () => {
  const savedPosition = sessionStorage.getItem('scrollPosition_wuspus_kontrasepsi')
  if (savedPosition) {
    setTimeout(() => {
      window.scrollTo({
      ...
      sessionStorage.removeItem('scrollPosition_wuspus_kontrasepsi')
    }, 100)
  }
}
...
function applyFilter() {
  saveScrollPosition()
  router.get('/posyandu/wuspus-kontrasepsi', {
...
// Format status KB Baru
const getKbBaruStatus = (value) => {
  if (value === 1 || value === '1' || value === true) return 'Ya'
  if (value === 0 || value === '0' || value === false) return 'Tidak'
  return '-'
}
...
function confirmDelete() {
  if (!selected.value?.id_wkp) {
    showToast('error', 'Data tidak valid')
    closeModal()
    return
  }

  const namaWuspus = selected.value.nama_wuspus || 'Data kontrasepsi'
  saveScrollPosition()
        ...
        <div class="filter-item search-item">
          ...
          <div class="search-wrapper">
            ...
            <input 
              ...
              class="search-input" 
              v-model="searchText" 
              placeholder="Cari Nama / NIK / Jenis Kontrasepsi..."
    ...
    <div class="modal-content">
      ...
      <div class="modal-highlight">
        ...
        <div class="modal-info-item">
          ...
          <div class="modal-info-value">
            <span class="kb-badge" :class="selected.kontrasepsi_baru == 1 ? 'kb-ya' : 'kb-tidak'">
              {{ getKbBaruStatus(selected.kontrasepsi_baru) }}
            </span>
          </div>
        </div>
      </div>
