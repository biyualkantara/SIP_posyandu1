...
function validateRow(row, index) {
  ...
  if (!row.id_imun) errors.push('Jenis imunisasi harus dipilih')
  if (!row.tgl_imun) errors.push('Tanggal imunisasi harus diisi')
...
function submit() {
  ...
  if (wuspusIds.length !== uniqueWuspusIds.length) {
    ...
    allErrors.push({
      row: 'Semua',
      errors: ['Tidak boleh memilih WUS/PUS yang sama untuk data yang berbeda']
    })
  }
  ...
  form.post('/posyandu/bumil-imun/store-multiple', {
    preserveScroll: true,
    onSuccess: () => {
      hideToast()
      openSuccess(`${form.rows.length} data imunisasi bumil berhasil disimpan`)
      setTimeout(() => {
        router.visit('/posyandu/bumil-imun')
      }, 1500)
    },
    onError: (errors) => {
      hideToast()
  ...
  <div class="data-container">
    ...
    <div class="header-section">
      <div class="header-left">
        <h1 class="page-title">Tambah Imunisasi Ibu Hamil (BUMIL)</h1>
        <p class="page-subtitle">Input multi data imunisasi ibu hamil</p>
      </div>
      <div class="header-right">
        <Link href="/posyandu/bumil-imun" class="btn-back">
          <span>←</span>
          <span>Kembali</span>
        </Link>
      </div>
      ...
      <div class="card-body">
        ...
        <div v-if="form.posyandu_id" class="info-alert">
          ...
          <span>
            ...
            Terpilih: <strong>{{ selectedWuspusIds.length }}</strong> | 
            Sisa: <strong>{{ wuspusOptions.length - selectedWuspusIds.length }}</strong>
          </span>
        </div>
        ...
        <form @submit.prevent="submit">
          ...
          <div v-if="form.posyandu_id" class="data-rows-section">
            <div class="section-header">
              <h6 class="section-title">Data Imunisasi BUMIL</h6>
              <div class="section-badge">
                <span class="badge-total">Total: {{ form.rows.length }} data</span>
                <span class="badge-available">Tersedia: {{ wuspusOptions.length - selectedWuspusIds.length }}</span>
              </div>
            </div>
              ...
              <div class="form-grid-2">
                <div class="form-field">
                  ...
                  <small v-if="getWuspusOptionsForRow(i).length === 0" class="text-warning">
                    ⚠️ Tidak ada WUS/PUS tersedia untuk baris ini
                  </small>
                </div>
