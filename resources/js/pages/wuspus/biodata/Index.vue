...
console.log('Data WUS/PUS:', props.data)
...
const saveScrollPosition = () => {
  ...
  sessionStorage.setItem('scrollPosition_wuspus', scrollPosition.value)
}

// Restore posisi scroll setelah kembali
const restoreScrollPosition = () => {
  const savedPosition = sessionStorage.getItem('scrollPosition_wuspus')
  if (savedPosition) {
    setTimeout(() => {
      window.scrollTo({
      ...
      sessionStorage.removeItem('scrollPosition_wuspus')
    }, 100)
  }
}
...
function confirmDelete() {
  if (!selected.value?.id_wuspus) {
    showToast('error', 'Data tidak valid')
    closeModal()
    return
  }

  const namaWuspus = selected.value.nama_wuspus || 'Data WUS/PUS'
  saveScrollPosition()
  ...
  router.delete(`/posyandu/wuspus/${selected.value.id_wuspus}`, {
    ...
    onSuccess: (page) => {
      ...
      if (page.props.flash?.success) {
        ...
        // Fallback message
        showToast('success', `Data "${namaWuspus}" berhasil dihapus!`)
      }
