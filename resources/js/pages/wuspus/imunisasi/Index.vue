...
console.log('Data Imunisasi WUS/PUS:', props.data)
...
const saveScrollPosition = () => {
  ...
  sessionStorage.setItem('scrollPosition_wuspus_imun', scrollPosition.value)
}

// Restore posisi scroll setelah kembali
const restoreScrollPosition = () => {
  const savedPosition = sessionStorage.getItem('scrollPosition_wuspus_imun')
  if (savedPosition) {
    setTimeout(() => {
      window.scrollTo({
      ...
      sessionStorage.removeItem('scrollPosition_wuspus_imun')
    }, 100)
  }
}
...
const columns = [
  { key: 'id_wuspus_imun', label: 'ID', sortable: true },
...
function applyFilter() {
  saveScrollPosition()
  router.get('/posyandu/wuspus-imun', {
...
function confirmDelete() {
  if (!selected.value?.id_wuspus_imun) {
    showToast('error', 'Data tidak valid')
    closeModal()
    return
  }

  const namaWuspus = selected.value.nama_wuspus || 'Data imunisasi'
  saveScrollPosition()
  
  router.delete(`/posyandu/wuspus-imun/${selected.value.id_wuspus_imun}`, {
    preserveScroll: true,
    onSuccess: (page) => {
      closeModal()
      ...
      if (page.props.flash?.success) {
        ...
        // Fallback message
        showToast('success', `Data imunisasi "${namaWuspus}" berhasil dihapus!`)
      }
    ...
    <div class="data-container">
      ...
      <div class="header-section">
        <div class="header-left">
          <h1 class="page-title">Imunisasi WUS/PUS</h1>
          <p class="page-subtitle">Kelola data imunisasi WUS/PUS</p>
        </div>
        <div class="header-right">
          <Link 
            href="/posyandu/wuspus-imun/create" 
            class="btn-create"
      ...
      <div class="table-section">
        ...
        <div v-if="rows.length === 0" class="empty-state">
          ...
          <p class="empty-description">Data imunisasi WUS/PUS belum tersedia. Silakan tambah data baru.</p>
        ...
        <DataTable v-else :columns="columns" :rows="rows" :perPage="10">
          ...
          <template #col-id_wuspus_imun="{ row }">
            <span class="id-badge">#{{ row.id_wuspus_imun }}</span>
          </template>
          ...
          <template #col-jns_imun="{ row }">
            <span class="imun-badge">{{ row.jns_imun || '-' }}</span>
          </template>

          <template #col-tgl_imun="{ row }">
            <span class="tanggal-text">{{ formatTanggal(row.tgl_imun) }}</span>
          </template>

          <template #col-actions="{ row }">
            <div class="action-group">
              <Link 
                :href="`/posyandu/wuspus-imun/${row.id_wuspus_imun}`"
                @click="handleLinkClick"
              >
              ...
              <Link 
                :href="`/posyandu/wuspus-imun/${row.id_wuspus_imun}/edit`"
                @click="handleLinkClick"
              >
    ...
    <div v-if="modalOpen" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content">
        ...
        <div class="modal-highlight">
          ...
          <div class="modal-info-item">
            <span class="modal-info-label">Jenis Imunisasi</span>
            <div class="modal-info-value">
              {{ selected.jns_imun || '-' }}
            </div>
          </div>
          ...
          <div class="modal-info-item">
            ...
            <div class="modal-info-value">
              {{ formatTanggal(selected.tgl_imun) }}
            </div>
          </div>
        </div>
