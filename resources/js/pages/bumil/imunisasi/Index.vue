...
const selectedPos = ref(props.filter?.pos ?? '') // tetap disimpan biar controller konsisten
  ...
  router.delete(`/posyandu/bumil-imun/${selected.value.id_bumil_imun}`, {
    ...
    onSuccess: () => {
      ...
      window.dispatchEvent(new CustomEvent("toast", {
        detail: { type: "success", message: "Data imunisasi berhasil dihapus!" }
      }))
    },
    onError: () => {
      window.dispatchEvent(new CustomEvent("toast", {
  ...
  <AdminLayout>
    <div class="mt-3 p-4 bg-white main-container">
      <div class="header-flex mb-3">
        <h1>Imunisasi Ibu Hamil</h1>
        <Link href="/posyandu/bumil-imun/create" class="btn btn-primary">+ Tambah</Link>
      </div>
      ...
      <DataTable :columns="columns" :rows="rows" :perPage="10">
        ...
        <template #col-actions="{ row }">
          <Link :href="`/posyandu/bumil-imun/${row.id_bumil_imun}`">
            <span class="bg-info p-3 mr-2 rounded-circle text-white" style="cursor:pointer;">
              <i class="icon-eye"></i>
            </span>
          </Link>

          <Link :href="`/posyandu/bumil-imun/${row.id_bumil_imun}/edit`">
