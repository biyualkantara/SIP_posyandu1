...
// Daftar kontrasepsi
const KONTRASEPSI_OPTIONS = [
  { label: 'PIL', value: 'PIL' },
  { label: 'SUNTIK', value: 'SUNTIK' },
  { label: 'IUD', value: 'IUD' },
  { label: 'IMPLAN', value: 'IMPLAN' },
  { label: 'MOW', value: 'MOW' },
  { label: 'MOP', value: 'MOP' },
  { label: 'KONDOM', value: 'KONDOM' }
]

// Opsi Ya/Tidak untuk kontrasepsi baru
...
function addRow() {
  // Cek apakah masih ada WUS/PUS yang tersedia
  const availableWuspusCount = wuspusOptions.value.length - selectedWuspusIds.value.length
  
  if (availableWuspusCount <= 0) {
...
function validateRow(row, index) {
  ...
  if (!row.id_wuspus) errors.push('WUS/PUS harus dipilih')
  if (!row.jns_kontrasepsi) errors.push('Jenis kontrasepsi harus dipilih')
  if (!row.tgl_ganti) errors.push('Tanggal ganti harus diisi')
  if (!row.kontrasepsi_baru) errors.push('Kontrasepsi baru harus diisi')
...
function submit() {
  ...
  form.post('/posyandu/wuspus-kontrasepsi/store-multiple', {
    ...
    onSuccess: () => {
      hideToast()
      openSuccess(`${form.rows.length} data kontrasepsi WUS/PUS berhasil disimpan`)
      setTimeout(() => {
        router.visit('/posyandu/wuspus-kontrasepsi')
      }, 1500)
    },
    onError: (errors) => {
      hideToast()
    ...
    <div class="data-container">
      ...
      <div class="header-section">
        <div class="header-left">
          <h1 class="page-title">Tambah Kontrasepsi WUS/PUS</h1>
          <p class="page-subtitle">Input multi data kontrasepsi WUS/PUS</p>
        </div>
        <div class="header-right">
          <Link href="/posyandu/wuspus-kontrasepsi" class="btn-back">
            <span>←</span>
            <span>Kembali</span>
          </Link>
        </div>
          ...
          <form @submit.prevent="submit">
            ...
            <div v-if="form.posyandu_id" class="data-rows-section">
              <div class="section-header">
                <h6 class="section-title">Data Kontrasepsi WUS/PUS</h6>
                <div class="section-badge">
                  <span class="badge-total">Total: {{ form.rows.length }} data</span>
                  <span class="badge-info">WUS/PUS tersedia: {{ wuspusOptions.length - selectedWuspusIds.length }}</span>
                </div>
              </div>
              ...
              <div v-for="(row, i) in form.rows" :key="i" class="data-card">
                ...
                <div class="form-grid-2">
                  ...
                  <div class="form-field">
                    ...
                    <div class="select-wrapper">
                      <VueSelect
                        v-model="row.jns_kontrasepsi"
                        :options="KONTRASEPSI_OPTIONS"
                        placeholder="Pilih Jenis Kontrasepsi"
                        class="vue-select-custom"
                      />
                    </div>
                  </div>
