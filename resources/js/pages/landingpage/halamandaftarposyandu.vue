        ...
        <div class="panel-main">
          ...
          <div class="filter-box">
            <div class="filter-grid">
              ...
              <!-- tombol harus turun sejajar input (bukan sejajar label) -->
...
const filters = reactive({ nama: "", status: "", alamat: "", kecamatan: "" });
...
:global(body.esip-posyandu-body .content) { padding: 0 !important; }
...
.filter-grid{
  ...
  gap:10px;
  align-items:start; /* penting: supaya tombol bisa "turun" pakai padding-top */
}
.fi label{
  display:block;
  font-size:11px;
...
.fi input:focus, .fi select:focus{
  border-color:#bfc7d8;
  box-shadow:0 0 0 3px rgba(140,154,210,.18);
}

/* tombol kanan: turun sejajar input */
.fi-actions{
  display:flex;
  flex-direction:column;
  gap:10px;
  padding-top: 23px; /* = tinggi label + jarak label */
}
...
.btn-primary{ background:#e9eaf5; color:#3a3d66; }
.btn-ghost{ background:#f1f2f6; color:#666; }
.btn-soft{
  background:#ececf6;
  color:#3a3d66;
  padding:0 14px;
...
.tbl tbody td.col-aksi .btn-pill{
  margin:0 auto;
}
...
/* Side stats */
.panel-side{ display:grid; gap:14px; }
.stat-card{
  background:#fff;
  border-radius:12px;
  padding:18px 16px;
...
@media (max-width: 992px){
  .posyandu-inner{ grid-template-columns: 1fr; }
  .filter-grid{ grid-template-columns: 1fr 1fr; }
  .fi-actions{ grid-column: 1 / -1; padding-top:0; flex-direction:row; }
  .fi-actions .btn-pill{ width:100%; }
