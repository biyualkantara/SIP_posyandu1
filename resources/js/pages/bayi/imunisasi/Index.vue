...
<AdminLayout>
  <div class="mt-3 p-4 bg-white main-container">
    <div class="header-flex mb-3">
      ...
      <Link href="/posyandu/bayi-imun/create" class="btn btn-primary">+ Tambah</Link>
    </div>

    <hr>

    <DataTable :columns="columns" :rows="rows" :perPage="10">
      <template #col-nama_bayi="{ row }">
        <span>{{ row.nik_bayi }} - {{ row.nama_bayi }}</span>
      </template>
  ...
  <div v-if="modalOpen" class="overlay-blur" @click.self="closeModal">
    <div class="modal-card">
      ...
      <div class="text-center">
        ...
        <div class="text-muted mt-1">{{ selected.nik_bayi }}</div>
