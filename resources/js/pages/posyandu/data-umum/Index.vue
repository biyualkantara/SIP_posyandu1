...
console.log('Data dari controller:', props.data)
...
const rows = computed(() => {
    ...
    // Jika data memiliki property data (pagination)
    ...
    router.delete(`/posyandu/data-umum/${selected.value.id_posyandu}`, {
        ...
        onSuccess: (page) => {
            ...
            if (page.props.flash?.success) {
                ...
                // Fallback message
                showToast('success', `Data "${namaPosyandu}" berhasil dihapus!`)
            }
    ...
    <div class="data-container">
        ...
        <div class="header-section">
            <div class="header-left">
                <h1 class="page-title">Data Umum Posyandu</h1>
                <p class="page-subtitle">Kelola data posyandu di wilayah Anda</p>
            </div>
            <div class="header-right">
                <Link 
                    href="/posyandu/data-umum/create" 
                    class="btn-create"
        ...
        <div class="table-section">
            ...
            <DataTable v-else :columns="columns" :rows="rows" :perPage="20">
                ...
                <template #col-id_posyandu="{ row }">
                    <span class="id-badge">#{{ row.id_posyandu }}</span>
                </template>
                ...
                <template #col-strata_psy="{ row }">
                    <span v-if="row.strata_psy" class="strata-badge" :class="`strata-${row.strata_psy.toLowerCase()}`">
                        {{ row.strata_psy }}
                    </span>
                    <span v-else class="text-muted">-</span>
                </template>

                <template #col-alamat_posyandu="{ row }">
                    <span class="alamat-text" :title="row.alamat_posyandu">
                        {{ formatValue(row.alamat_posyandu) }}
                    </span>
                </template>

                <template #col-pj_umum="{ row }">
                    {{ formatValue(row.pj_umum) }}
                </template>

                <template #col-kader_aktif="{ row }">
                    <span class="kader-badge">{{ row.kader_aktif || 0 }} Kader</span>
                </template>

                <template #col-actions="{ row }">
                    <div class="action-group">
                        <Link 
                            :href="`/posyandu/data-umum/${row.id_posyandu}`"
                            @click="handleLinkClick"
                        >
                        ...
                        <Link 
                            :href="`/posyandu/data-umum/${row.id_posyandu}/edit`"
                            @click="handleLinkClick"
                        >
