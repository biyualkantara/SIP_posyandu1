...
const imunOptions = computed(() => {
  return props.imun.map(i => ({
    label: i.jns_imun,
    value: i.id_imun  // Ini sudah integer dari database
  }))
})
...
function submit() {
  ...
  form.post('/posyandu/wuspus-imun/store-multiple', {
    ...
    onSuccess: () => {
      hideToast()
      openSuccess(`${form.rows.length} data imunisasi WUS/PUS berhasil disimpan`)
      setTimeout(() => {
        router.visit('/posyandu/wuspus-imun')
      }, 1500)
    },
    onError: (errors) => {
      hideToast()
    ...
    <div class="data-container">
      ...
      <div class="header-section">
        <div class="header-left">
          <h1 class="page-title">Tambah Imunisasi WUS/PUS</h1>
          <p class="page-subtitle">Input multi data imunisasi WUS/PUS</p>
        </div>
          ...
          <form @submit.prevent="submit">
            ...
            <div v-if="form.posyandu_id" class="data-rows-section">
              <div class="section-header">
                <h6 class="section-title">Data Imunisasi WUS/PUS</h6>
                <div class="section-badge">
                  <span class="badge-total">Total: {{ form.rows.length }} data</span>
                  <span class="badge-info">WUS/PUS tersedia: {{ wuspusOptions.length }}</span>
                </div>
              </div>
